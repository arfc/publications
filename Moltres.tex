...
\Cref{fig:intra_strong_scaling} shows Moltres performance, i.e. seconds per element vs. number of cores on 1 node
...
\Cref{fig:extra_strong_scaling} shows Moltres strong scaling up to 768 processors which takes into account communication costs 
...
used to solve a bigger problem which would not fit in memory on a single node. Thus, the weak scaling measurement is justification
 ...
 keeping the workload constant at 581, 985, 1970 and 3940 elements per core. \Cref{fig:intra_weak} shows Moltres weak scaling 
performance measured for 1 \ldots 32 core within one Blue Waters node and \Cref{fig:extra_weak} demonstrates performance for 
