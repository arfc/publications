...
%\bottomrule
%\end{tabular}
%  \caption{Materials flows rates during \gls{MSBR} online reprocessing [1/s*cm$^3$].}
%  \label{tab:rates}
%\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[hb] % replace 't' with 'b' to force it to be on the bottom
  ...
  \vspace{-0.8em}
\end{figure}
From the graph below we can see that to keep reactor critical higher $^{233}$U flow rate required for first 400 days of operation and after that much smaller uranium-233 flow rate from the protactinium decay tank needed. The $^{232}$Th rate slightly decreases over 4 years of operation due to other than $^{233}$U fissile materials accumulation.

\begin{figure}[hb] % replace 't' with 'b' to force it to be on the bottom
  \centering
  \vspace{-0.8em}
  \includegraphics[width=1.03\linewidth]{rates_outflow.png}
  \caption{Materials flows rates for the protactinium decay tank during \gls{MSBR} online reprocessing.}
  \vspace{-0.6em}
  \label{fig:outflow}
  \vspace{-0.8em}
\end{figure}
As shown in Figure~\ref{fig:outflow}, the tank for protactinium decay accumulates $^{233}$Pa approximately 200 days and fresh fuel ($^{233}$U) flow established also after 200 days. Uranium produced in the tank by protactinium decay is separated by circulation of the salt through a flourinator. The fully processed molten salt, on its way back to the primary loop, has uranium added at the rate required to maintain or adjust the fissile material concentration and, hence, the reactivity, in the reactor core as desired.
