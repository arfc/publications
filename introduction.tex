...
There also are studies on regional (European) transmutation and partitioning with fast reactors
and \gls{ADS} to reduce disposal burden \cite{fazio_study_2013, martinez-val_pateros_2009}.
There has been little attention in utilizing legacy \gls{SNF} from other
...
The second scenario utilizes the obtained \gls{SNF} inventory in \gls{EU} at 2050,
...
is mixed with depleted uranium to produce \gls{MOX}. Note that the plutonium
from both spent fuel types are of 'good quality' (~70\% fissile, ~25\% Pu240),
and the \gls{MOX} fuel is fabricated in the simulation to have an appropriate
neutronic property \cite{huff_fundamental_2016}.
